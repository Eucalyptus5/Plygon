    ...
    'walk: for (tag, path) in shards {
        ...
        for (rec_idx, rec) in train_dump::read_records(path.to_str().unwrap())?.into_iter().enumerate() {
            ...
            if phase_asym || s.pending_actions[0] != s.pending_actions[1] {
                ...
                if phase_asym {
                    st.phase_found += 1;
                    st.phase_differed += differed as u64;
                } else {
                    st.pending_found += 1;
                    st.pending_differed += differed as u64;
                }
                // both sides read an empty legal set, so equality is forced and
                // carries no orientation signal
                if !differed && block_is_zero(&mirrored) {
                    st.both_empty += 1;
                }
            }
            st.records += 1;
...
fn print_verify(st: &VerifyStats) {
    ...
    println!(
        "  negative control  phase-asym found {} differed {} | pending-asym found {} differed {}",
        st.phase_found, st.phase_differed, st.pending_found, st.pending_differed
    );
    println!(
        "                    of {} non-differing, {} had an empty legal set on both sides",
        st.phase_found + st.pending_found - st.phase_differed - st.pending_differed,
        st.both_empty
    );
...
"#;
    ...
    fn verify_action_dense_passes_a_clean_conversion_and_names_the_corrupted_row() {
        ...
        assert_eq!(st.phase_differed, 32);
        // a pending-asym record that does not differ must not gate, and its two
        // reads are populated rather than forced empty
        assert_eq!(st.pending_found, 16);
        assert_eq!(st.pending_differed, 0);
        assert_eq!(st.both_empty, 0);
    ...
    #[test]
    fn an_empty_legal_set_on_both_sides_is_found_but_not_differed_and_does_not_fail() {
        let (base, _) = verify_base();
        let mut p1 = base;
        p1.phase = pkmn_engine::state::PHASE_SWITCH_P1;
        let mut empty_both = p1;
        empty_both.sides[1].team[1].current_hp = 0;

        // side 1 is off-turn under P1 and its bench is gone, so the mirrored read
        // has no legal switch either and equality is forced
        let mirrored = action_features(&features::mirror(&empty_both), 0);
        assert_eq!(mirrored, action_features(&empty_both, 1));
        assert!(mirrored.iter().flatten().all(|&v| v == 0.0), "both reads must be all-zero");
        assert_ne!(
            action_features(&features::mirror(&p1), 0),
            action_features(&p1, 1),
            "with a live bench the same state does differ"
        );

        let dir = tmp_dir("verifyempty");
        let out = tmp_dir("verifyempty_out");
        dump_verify_shards(&dir, [base, p1], [base, empty_both]);
        convert_for_verify(&dir, &out);
        let st = verify_action_dense(&dir, &out, None).unwrap();
        assert_eq!(st.phase_found, 32);
        assert_eq!(st.phase_differed, 16);
        assert_eq!(st.both_empty, 16);
        assert!(st.passed(), "a forced-empty pair must not fail the run: {st:?}");
