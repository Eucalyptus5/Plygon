...
enum Kind { Random, Greedy, Mcts }

fn parse_kind(s: &str) -> Kind {
    match s { "random" => Kind::Random, "greedy" => Kind::Greedy, "mcts" => Kind::Mcts, _ => panic!("unknown policy {s}") }
}
...
fn choose(kind: Kind, state: &BattleState, teams: &TeamData, side: usize, rng: &mut Lcg, time_ms: u64, seed: u64) -> u8 {
    match kind {
...
fn play(p1: Kind, p2: Kind, t1: &[MonJson], t2: &[MonJson], game_seed: u64, time_ms: u64) -> f64 {
    ...
    for turn in 0..500u64 {
        ...
        let a1 = if legal_actions(&state, 0).count > 0 { choose(p1, &state, &teams, 0, &mut pol_rng, time_ms, s1) } else { ACTION_STRUGGLE };
        let a2 = if legal_actions(&state, 1).count > 0 { choose(p2, &state, &teams, 1, &mut pol_rng, time_ms, s2) } else { ACTION_STRUGGLE };
        match state.phase {
...
fn main() {
    ...
    for g in 0..games {
        ...
        let v = if g % 2 == 0 {
            play(p1, p2, &fixture.teams[ta], &fixture.teams[tb], seed ^ g, time_ms)
        } else {
            1.0 - play(p2, p1, &fixture.teams[ta], &fixture.teams[tb], seed ^ g, time_ms)
        };
        score += v;
