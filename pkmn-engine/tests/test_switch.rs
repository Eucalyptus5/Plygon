...
fn test_stealth_rock() {
    ...
    switch_in(&mut state, &TeamData::default(), 0, 1); // switch to idx 1
...
fn test_spikes() {
    ...
    switch_in(&mut state2, &TeamData::default(), 0, 1);
    assert_eq!(state2.sides[0].team[1].current_hp, 300 - (300 / 6));
    
    // 3 layers
    let mut state3 = setup();
    state3.sides[0].side_conditions.spikes = 3;
    switch_in(&mut state3, &TeamData::default(), 0, 1);
...
fn test_hazard_ko_triggers_switch() {
    ...
    execute_switch_turn(&mut state, &TeamData::default(), 5, 0, &mut rng);
