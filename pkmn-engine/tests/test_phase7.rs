...
fn test_belly_drum_boost_and_hp_cost() {
    ...
    deal_damage(&mut state, 0, 0, max_hp / 2);
    let current_atk_boost = state.sides[0].active.boosts[ATK];
    apply_boost(&mut state, 0, ATK, 6 - current_atk_boost);
...
fn test_taunt_blocks_status_moves() {
    ...
    execute_move(&mut state, &TeamData::default(), 1, 269, 0, &mut dummy_rng);
...
fn test_encore_locks_to_last_move() {
    ...
    execute_move(&mut state, &TeamData::default(), 1, 227, 0, &mut dummy_rng);
...
fn test_trick_swaps_items() {
    ...
    set_item(&mut state, 0, 0, 100); // Side 0 gets item 100
    set_item(&mut state, 1, 0, 200); // Side 1 gets item 200

    // Apply Trick effect directly
    let item_a = state.sides[0].team[0].item_id;
    let item_d = state.sides[1].team[0].item_id;
    set_item(&mut state, 0, 0, item_d);
    set_item(&mut state, 1, 0, item_a);
...
fn test_aegislash_forme_toggle() {
    ...
    apply_battle_forme(&mut state, &teams, 0, 1103);
...
fn test_mid_turn_faint_triggers_switch_phase() {
    ...
    if state.phase == PHASE_SWITCH_P1 {
        ...
        execute_switch_turn(&mut state, &TeamData::default(), switch_action, 0, &mut dummy_rng);
...
fn test_both_faint_triggers_switch_both() {
    ...
    execute_switch_turn(&mut state, &TeamData::default(),
...
fn test_forced_switch_resumes_turn() {
    ...
    execute_switch_turn(&mut state, &TeamData::default(), 0, ACTION_SWITCH_0 + 1, &mut dummy_rng);
...
fn test_faint_after_move2_triggers_switch() {
    ...
    execute_switch_turn(&mut state, &TeamData::default(), ACTION_SWITCH_0 + 1, 0, &mut dummy_rng);
...
fn test_trick_room_no_affect_priority() {
    ...
    execute_turn(&mut state, &TeamData::default(), 2, 0, &mut dummy_rng);
...
fn test_encore_forces_move() {
    ...
    set_volatile(&mut state, 0, VOL_MOVED_THIS_TURN);
...
fn test_disable_blocks_move() {
    ...
    execute_move(&mut state, &TeamData::default(), 1, 50, 0, &mut dummy_rng);
...
fn test_disable_duration_turn_order() {
    ...
    execute_move(&mut state2, &TeamData::default(), 1, 50, 0, &mut dummy_rng);
...
fn test_taunt_duration_turn_order() {
    ...
    set_volatile(&mut state2, 0, VOL_MOVED_THIS_TURN);

    execute_move(&mut state2, &TeamData::default(), 1, 269, 0, &mut dummy_rng);
...
fn test_torment_blocks_repeat() {
    ...
    set_volatile(&mut state, 0, VOL_TORMENT);
...
fn test_torment_clears_on_switch() {
    ...
    pkmn_engine::state::switch::switch_out(&mut state, &TeamData::default(), 0);
    pkmn_engine::state::switch::switch_in(&mut state, &TeamData::default(), 0, 1);
...
fn test_torment_via_move() {
    ...
    execute_move(&mut state, &TeamData::default(), 1, 259, 0, &mut dummy_rng);
...
fn test_healing_wish_user_faints() {
    ...
    execute_move(&mut state, &TeamData::default(), 0, 361, 0, &mut dummy_rng);
