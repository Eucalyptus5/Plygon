...
def gen_call_family():
    """Generate gen_call_family.rs — sorted &[u16] move-ID slices for the Call* family.

    Seven sidecars: METRONOME_OK, SLEEP_TALK_FAIL, ASSIST_FAIL, COPYCAT_FAIL,
    ME_FIRST_FAIL, MIRROR_MOVE_OK, ENCORE_FAIL.

    Filter rules:
      - METRONOME_OK mirrors Showdown's Metronome onHit filter:
          (!isNonstandard || isNonstandard === 'Unobtainable') && flags.metronome
      - The other six sidecars include moves the engine has data for:
        not in {CAP, LGPE, Unobtainable, Gigantamax} — i.e. None or Past.
        (Past moves like Assist, Me First, Mirror Move can appear in last_move
        records, so their flags must be reachable via the sidecars.)
    """
    text = (SD / "moves.ts").read_text(encoding="utf-8")
    entries = parse_ts_object(text)

    sidecar_order = [
        "METRONOME_OK", "SLEEP_TALK_FAIL", "ASSIST_FAIL",
        "COPYCAT_FAIL", "ME_FIRST_FAIL", "MIRROR_MOVE_OK", "ENCORE_FAIL",
    ]
    sidecars = {name: [] for name in sidecar_order}
    ...
    for key, block in entries.items():
        ...
        nonstd_val = extract_field(block, "isNonstandard")
        nonstd = nonstd_val.strip("'\"") if nonstd_val else None

        sd_flags = extract_flags(block)

        if "metronome" in sd_flags and (nonstd is None or nonstd == "Unobtainable"):
            sidecars["METRONOME_OK"].append(num)

        engine_has_data = nonstd is None or nonstd == "Past"
        if engine_has_data:
            if "nosleeptalk" in sd_flags:
                sidecars["SLEEP_TALK_FAIL"].append(num)
            if "noassist" in sd_flags:
                sidecars["ASSIST_FAIL"].append(num)
            if "failcopycat" in sd_flags:
                sidecars["COPYCAT_FAIL"].append(num)
            if "failmefirst" in sd_flags:
                sidecars["ME_FIRST_FAIL"].append(num)
            if "mirror" in sd_flags:
                sidecars["MIRROR_MOVE_OK"].append(num)
            if "failencore" in sd_flags:
                sidecars["ENCORE_FAIL"].append(num)

    for name in sidecars:
        sidecars[name] = sorted(set(sidecars[name]))
    ...
    for name in sidecar_order:
        ids = sidecars[name]
        out.append(f"/// {len(ids)} moves carrying the corresponding Showdown flag. Sorted ascending for binary_search.")
        out.append(f"pub static {name}: &[u16] = &[")
    ...
    (OUT / "gen_call_family.rs").write_text("\n".join(out), encoding="utf-8")
    total = sum(len(v) for v in sidecars.values())
    sizes = ", ".join(f"{name}={len(sidecars[name])}" for name in sidecar_order)
...
def gen_abilities():
    """Generate gen_abilities.rs from abilities.ts.

    Emits a bitset over ability nums for Showdown's `notransform` flag
    (abilities ignored while the holder is transformed — sim/pokemon.ts
    ignoringAbility).
    """
    text = (SD / "abilities.ts").read_text(encoding="utf-8")
    entries = parse_ts_object(text)
    ...
    for key, block in entries.items():
        ...
        if num is None or num <= 0:
            continue
        if "notransform" in extract_flags(block):
            flagged.append(num)

    flagged.sort()
    words = [0] * ((max(flagged) >> 6) + 1)
    for n in flagged:
        words[n >> 6] |= 1 << (n & 63)

    out = []
    out.append("// @generated by codegen.py — do not edit.")
    out.append("// Abilities carrying Showdown's `notransform` flag (data/abilities.ts):")
    out.append("// ignored while the holder is transformed (sim/pokemon.ts ignoringAbility).")
    out.append("// Bit index = ability num.")
    out.append(f"pub(crate) const NOTRANSFORM_ABILITY_BITS: [u64; {len(words)}] = [")
    for w in words:
        out.append(f"    0x{w:016x},")
    out.append("];")
    (OUT / "gen_abilities.rs").write_text("\n".join(out) + "\n", encoding="utf-8")
    print(f"Wrote {OUT / 'gen_abilities.rs'}: {len(flagged)} notransform abilities — {flagged}")
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Generate gen_moves.rs, gen_items.rs, and gen_call_family.rs")
    ...
    p.add_argument("--abilities", action="store_true", help="regenerate gen_abilities.rs only")
    ...
    both = (not args.moves and not args.items and not args.call_family
            and not args.abilities and not args.species_nfe)
